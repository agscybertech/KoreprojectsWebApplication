...
Partial Class WebControls_WebUserControlScopesPendingGrid
    ...
    Protected Sub Page_Load(ByVal sender As Object, ByVal e As System.EventArgs) Handles Me.Load
        ...
        If Session("IsGroupView") Then
            rptScopesPending.Visible = True
            gvScopesPending.Visible = False
        Else
            rptScopesPending.Visible = False
            gvScopesPending.Visible = True
        End If
        'End If
    End Sub
    ...
    Private Sub LoadGroupData()
        ...
        dsGroup = m_ScopeService.GetWorksheetGroupsByScopeIdScopeItemStatusUserId(m_ScopeID, 1, m_LoginUserId)
        ...
        rptScopesPending.DataSource = dsGroup.Tables(0)
        rptScopesPending.DataBind()
    End Sub

    Protected Sub rptScopesPending_ItemDataBound(ByVal sender As Object, ByVal e As System.Web.UI.WebControls.RepeaterItemEventArgs) Handles rptScopesPending.ItemDataBound
        If (e.Item.ItemType = ListItemType.Item Or e.Item.ItemType = ListItemType.AlternatingItem) Then
            Dim gvGroupScopesPending As GridView = CType(e.Item.FindControl("gvScopesPending"), GridView)
            If Not gvGroupScopesPending Is Nothing Then
                Dim row As DataRowView = CType(e.Item.DataItem, DataRowView)
                gvGroupScopesPending.DataSource = m_ScopeService.GetScopeItemsByScopeIdScopeItemStatusUserIdGroupId(m_ScopeID, 1, m_LoginUserId, Convert.ToInt64(row("WorksheetGroupId")))
                gvGroupScopesPending.DataBind()

                'If Request.Url.ToString.ToLower.IndexOf("/printscope.aspx") > 0 Then
                '    gvGroupScopesPending.Columns(9).Visible = False
                '    gvGroupScopesPending.Columns(7).Visible = False
                '    gvGroupScopesPending.Columns(6).Visible = False
                '    If Request.QueryString("act") <> "" Then
                '        If m_Cryption.Decrypt(Request.QueryString("act"), m_Cryption.cryptionKey) = "ShowRate" Then
                '            gvGroupScopesPending.Columns(7).Visible = True
                '            gvGroupScopesPending.Columns(6).Visible = True
                '        End If
                '    End If
                'End If
                ...
                If blnScopePricing Then
                    gvGroupScopesPending.Columns(7).Visible = True
                    gvGroupScopesPending.Columns(8).Visible = True
                Else
                    gvGroupScopesPending.Columns(7).Visible = False
                    gvGroupScopesPending.Columns(8).Visible = False
                End If
            End If

            ' Retrieve the pager row.
            Dim pagerRow As GridViewRow = gvGroupScopesPending.BottomPagerRow
            ...
            If Not pageList Is Nothing Then
                ...
                For i = 0 To gvGroupScopesPending.PageCount - 1
                    ...
                    If i = gvGroupScopesPending.PageIndex Then
            ...
            If Not pageLabel Is Nothing Then
                ...
                Dim currentPage As Integer = gvGroupScopesPending.PageIndex + 1

                ' Update the Label control with the current page information.
                pageLabel.Text = "Showing Page: " & currentPage.ToString() & _
                    " of " & gvGroupScopesPending.PageCount.ToString()

            End If

            If gvGroupScopesPending.Rows.Count > 0 Then
                If gvGroupScopesPending.PageIndex = 0 Then
                    If Not pagePre Is Nothing Then
                        pagePre.Visible = False
                    End If
                ElseIf gvGroupScopesPending.PageCount - 1 = gvGroupScopesPending.PageIndex Then
    ...
    Private Sub LoadScopesData()
        ...
        If blnScopePricing Then
            gvScopesPending.Columns(7).Visible = True
            gvScopesPending.Columns(8).Visible = True
        Else
            gvScopesPending.Columns(7).Visible = False
            gvScopesPending.Columns(8).Visible = False
        End If
    End Sub
