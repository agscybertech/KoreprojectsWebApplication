<%@ Control Language="VB" AutoEventWireup="false" CodeFile="WebUserControlNoteGrid.ascx.vb" Inherits="WebUserControlNoteGrid" %>
<asp:GridView ID="gvNotes" runat="server" AllowPaging="True" 
    ...
    <Columns>
        ...
        <asp:TemplateField HeaderText="ACTION">
            ...
            <ItemTemplate>
                <a href="Note.aspx?id=<%# m_Cryption.encrypt(Eval("Owner", "{0}") & "-" & Eval("UserNoteId", "{0}"),m_Cryption.cryptionKey) %>" rel="#overlaynote" style="text-decoration:none;display:none;">EDIT</a>
                <a class="form_popup" href="Note.aspx?id=<%# m_Cryption.encrypt(Eval("Owner", "{0}") & "-" & Eval("UserNoteId", "{0}"),m_Cryption.cryptionKey) & string.format("&{0}",Datetime.Now.ToString("yyyyddMMhhmmss")) %>" style="text-decoration:none;">EDIT</a>
            </ItemTemplate>
        </asp:TemplateField>
    </Columns>
