                ...
                For i As Integer = 0 To dsTimesheets.Tables.Count - 1
                    If dsTimesheets.Tables(i).Rows.Count > 0 Then
                        ...
                        For Each rData As DataRow In dsTimesheets.Tables(i).Rows
                            sTable = sTable & "<tr><td>" & rData("ID") & "</td><td>" & rData("ProjectName") & "</td><td>" & rData("Job") & "</td><td>" & CDate(rData("EntryDate")).ToString("d MMM yyyy") & "</td><td>" & rData("TotalHours") & "</td><td style='text-align:left'>" & rData("Description") & "</td>" & If(dsTimesheets.Tables(i).Select("ProjectOwnerUserId=" & m_LoginUser.UserId).Length > 0, "<td class=""editaction"">" & String.Format("<a class='form_popup' href='Timesheet.aspx?TimeSheetEntryID={0}'>EDIT</a>" & "</td>", rData("TimesheetEntryId")), "") & "</tr>"
