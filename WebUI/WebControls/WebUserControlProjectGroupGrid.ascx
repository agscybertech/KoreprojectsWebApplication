<%@ Control Language="VB" AutoEventWireup="false" CodeFile="WebUserControlProjectGroupGrid.ascx.vb" Inherits="WebControls_WebUserControlProjectGroupGrid" %>
<asp:GridView ID="gvProjectGroup" runat="server" AllowPaging="false" 
    ...
    <Columns>
        <asp:TemplateField Visible="false">
            ...
            <ItemTemplate>
                <img src='<%# showFile(Eval("Identifier", "{0}"), Eval("Logo", "{0}")) %>' style='<%# isFileExisted(Eval("Identifier", "{0}"), Eval("Logo", "{0}")) %>' width="50" border="0">
            </ItemTemplate>
        </asp:TemplateField>
        <asp:BoundField DataField="Name" HeaderText="Area" SortExpression="Name" ItemStyle-Width="140" ItemStyle-HorizontalAlign="left" />
        <asp:TemplateField HeaderText="ACTION">
            <ItemStyle HorizontalAlign="Center" Width="40" />
            <ItemTemplate>
                <a class="form_popup" href="ProjectGroup.aspx?id=<%# m_Cryption.encrypt(Eval("ProjectGroupId", "{0}"),m_Cryption.cryptionKey) & string.format("&{0}",Datetime.Now.ToString("yyyyddMMhhmmss")) %>" style="text-decoration:none;">EDIT</a>
            </ItemTemplate>
        </asp:TemplateField>
    </Columns>
